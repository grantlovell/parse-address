...
//      $this->controller();
    ...
    public function check($property = null)
    {
        ...
        if (strlen(trim($this->$property)) < 1) {
            return false;
        }
    ...
    public function isCity($string = null, &$parts = null)
    {
        ...
        if (in_array($temp, $this->_cities[$this->state])) {
            ...
            if ($this->_debug) {
                echo "<br>isCity chk for multi-word city ";
            }
            if (!empty($parts)) {
                $key = array_search($temp, $parts);
                if ($this->_debug) {
                    echo "city loop: ".$key.'-'.$temp.'-';
                }
    ...
    public function isCountry($string = null)
    {
        ...
        if (isset($this->_countries[$temp])) {
            return $string;
        }
        if (isset($this->_countries_reversed[$temp])) {
            return $string;
        }
    ...
    public function isDirection($string = null)
    {
        ...
        if (isset($this->_directions[$temp])) {
            return $string;
        }
        if (isset($this->_directions_reversed[$temp])) {
            return $string;
        }
    ...
    public function isState($string = null)
    {
        ...
        if (isset($this->_states[$temp])) {
            return $string;
        }
        if (isset($this->_states_reversed[$temp])) {
            return $string;
        }
    ...
    public function isStreetType($string = null)
    {
        ...
        //reasons to believe that this is a country
        if (isset($this->_streets[$temp])) {
            return $string;
        }
        if (isset($this->_streets_reversed[$temp])) {
            return $string;
        }
    ...
    protected function parseArray()
    {
        ...
        foreach ($this->_clean_address as $key => $part) {
            ...
            if ($this->parseSet("postal_code", $part)) {
                continue;
            }
            if ($this->parseSet("country", $part)) {
                continue;
            }
            if ($this->parseSet("state", $part)) {
                continue;
            }
            if ($this->parseSet("city", $part)) {
                continue;
            }

            //parse in a new manor if
            if ($temp = $this->parseStreet($part)) {
                continue;
            }
    ...
    protected function parseString()
    {
        ...
        foreach (array_reverse($parts, true) as $key => $part) {
            ...
            if ($this->_debug) {
                echo "<br>parsing: ".$key.' - '.$part.'  ('.$max_size.')';
            }
            ...
            if (is_null($this->postal_code)
                ...
                if ($this->_debug) {
                    echo "<br>setting postal code - ".$part;
                }
                unset($parts[$key]);
                continue;
            }
    ...
    public function parseStreet($string = null)
    {
        //initializing variables
        if ($this->_debug) {
            echo "<br><br>Entering parseStreet with: ".$string;
        }
        $leftovers = "";
        $string = $this->clean($string);
        ...
        foreach ($parts as $key => $part) {
            ...
            if ($this->isStreetNumber($part)
                ...
                if ($this->_debug) {
                    echo "<br>unset strNu: ".$key.' - '.$part;
                }
                unset($parts[$key]);
            } elseif ($this->isStreetType($part)
                && ($this->parseSet("street_type", $part))
                && ($key > 1)) {
                // type must be after st name
                //STREET TYPE
                if ($this->_debug) {
                    echo "<br>unset strType: ".$key.' - '.$part;
                }
                unset($parts[$key]);
                //continue;
    ...
    protected function parseSet($property = null, $value = null)
    {
        ...
        if ($this->fireMethod($method, $this->$property)) {
            return false;
        }
    ...
    public function set($property = null, $value = null, $force = false)
    {
        ...
        if (! $force && ! ($valid = $this->fireMethod($method, $value))) {
            return false;
        }
    ...
    public function toString()
    {
        ...
        $full  = $this->street_number()." ".$this->streetName()." ".$this->street_type();
